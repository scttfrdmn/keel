...
- **Gate P2:** `spill-audit` reports **0 accumulator spills** in the steady-state K-loop, AND the raw microkernel (packed inputs, no blocking) hits **≥55% of theoretical peak** (peak = cores·freq·2 FMA ports·16 lanes·2 flops; the harness computes it from CPUID + measured frequency). If either fails after kernel-shaping and one shrink step: STOP, write `docs/spill-report.md` with the ssa.html evidence, and surface the decision (file upstream vs. avo fallback vs. accept AVX2 shapes). Do not silently proceed to Phase 3 on a broken kernel.
...
4. **Benchmarks are tests:** perf gates run in CI-mode locally (`go test -run=NONE -bench=Gate -benchtime=3x`) and the gate script parses the numbers. A silent 2× regression should fail a gate, not await a human.
...
| Perf numbers flattering on one machine | Bench harness records CPU model, frequency governor, and theoretical peak alongside every result |
...
7. **Honesty in benchmarks.** Every benchmark result you report includes: CPU model, measured vs theoretical peak, and the OpenBLAS reference number from the same machine when the cgo dev-harness is available. If OpenBLAS is not installed, say so and report percent-of-theoretical-peak only. Never present a number without its denominator.
