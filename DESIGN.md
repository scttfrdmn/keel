...
- **On an issue-bound host the P3 denominator is `min(same-host OpenBLAS, roofline × measured peak)`** (same ruling; cause is #17/#18, `docs/toolchain-notes.md` T12). OpenBLAS on Skylake-X is hand assembly whose K-loop folds accumulation and an embedded broadcast into single FMAs — instructions the intrinsic layer provably cannot emit — so it lives *above* the front-end ceiling that caps keel's kernels. Asking keel for ≥60% of that is asking the decode stage, not the kernel, and P2 already adjudicated that root cause rather than leaving it as keel's defect. So keel is asked for ≥60% of what its instruction stream *as-lowered* can express. Four properties keep this from being a dispensation:
  ...
  - **The roofline is the one belonging to the shape `Sgemm` actually ran**, read from the `keel-bench-kern` marker of the very run that produced the ratio, and it carries P2's anti-vacuity shape guard unchanged: a shape more than 5% above the sweep's best zero-spill insns/FMA is refused a roofline outright and the host reverts to the plain OpenBLAS denominator. A fatter kernel cannot buy itself a lower bar — which is not hypothetical here, because the currently-dispatched 4×32 (6.250 insns/FMA against a 4.438 sweep best) *is* refused, and only the 2×32 the sentinel gates (4.625) is inside the guard. See issue #24: on an issue-bound host the shape dispatch selects and the shape that measures best are not the same shape, and that is a keel problem, not a denominator problem.
