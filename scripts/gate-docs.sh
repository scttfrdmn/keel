...
shell_files() {
  git ls-files -co --exclude-standard | while read -r f; do
    [ -f "$f" ] || continue
    case "$f" in archive/*) continue ;; *.sh) printf '%s\n' "$f"; continue ;; esac
...
stage_ratio() {
  ...
  local sh lib tool toollib bench benchlib ratio aratio oratio app libnet toolt arsh arlib
  sh="$(shell_files | while read -r f; do wc -l < "$f"; done | awk '{n+=$1} END{print n+0}')"
  lib="$(git ls-files -co --exclude-standard '*.go' | { grep -vE '_test\.go$|^archive/' || true; } | while read -r f; do wc -l < "$f"; done | awk '{n+=$1} END{print n+0}')"
  arsh="$(git ls-files -co --exclude-standard 'archive/*' | while read -r f; do [ -f "$f" ] || continue; case "$f" in *.sh) wc -l < "$f"; continue ;; esac; read -r l < "$f" 2>/dev/null || continue; case "$l" in '#!'*sh | '#!'*sh\ *) wc -l < "$f" ;; esac; done | awk '{n+=$1} END{print n+0}')"
  arlib="$(git ls-files -co --exclude-standard 'archive/*.go' | { grep -v '_test\.go$' || true; } | while read -r f; do wc -l < "$f"; done | awk '{n+=$1} END{print n+0}')"
  ...
  info "apparatus = shell + tools/ + bench/ INCLUDING their tests; library = *.go net of its tests. The asymmetry is deliberate and only defensible stated: the ratio measures maintenance burden against shipped substance, and a test of an instrument is burden. Both terms count tracked AND untracked-not-ignored, since a counter that polices new shell must see it before it is committed. internal/spill's audit instrument stays on the library side, disclosed. RESTATED 2026-08-31 (#131): tools/*_test.go was in NEITHER term, ${toolt} lines of it here; folding it in reads ${oratio}x under the pre-fold definition against ${aratio}x under this one, which is a DEFINITION correction of one tree and not a regression. The same fold measured 2.69x -> 2.79x on 2026-08-30 when the hole was found; both ends have moved with the tree since, which is why this line computes them instead of quoting them. RESTATED AGAIN 2026-08-31 (#131, ruled): the shell term counts shell BY CONTENT, the '*.sh' glob unioned with every tracked-or-untracked file whose shebang names a shell, because scripts/fakessh's 97 lines of bash were invisible to the glob while its own 189-line test was counted in full -- a ledger measuring by file extension. +97 at this rev, booked in docs/apparatus-ledger.md as a definition correction and not a spend. RESTATED AGAIN 2026-09-02 (ruled): archive/ is OUT OF BOTH TERMS, -${arsh} shell and -${arlib} library at this rev. THE CHANGEOVER READING is PINNED to be6cca9, the commit that made the change, and reads 16725 / 8993 / 1.86x under the old definition against 16468 / 8964 / 1.84x under this one; docs/apparatus-ledger.md's be6cca9 row and CHANGELOG.md carry the same pair. It is pinned because it is a historical reading of one tree and not a property of this one: this line used to COMPUTE it as shell+archive, which drifts upward with every campaign archived, so by 97a21f4 it printed 17093 while claiming to be 'printed once and only in the commit that makes it' -- a figure a reader would compare against the ledger's 16725 and read as a regression. What this run measures under the definition in force is ${sh} / ${lib} / ${ratio}x. WHY: the ratio measures code-maintenance burden, and an archived driver is neither apparatus nor library but EVIDENCE -- it is never edited again, it is frozen at the hash that produced its data by the same rule that freezes the tree for a run, and it accumulates once per campaign forever. Leaving it in made the ratio drift on accumulation rather than on code, which is the one thing a burden metric may not do. archive/ stays fully inside every OTHER check here -- citation resolution, the digest witnesses, the numbers extraction -- and is excluded from THIS ratio only"
